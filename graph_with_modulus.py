...
def delet_rows_after_break(df):
    ...
    df2=df['Deformação'].iloc[-100:].copy()
...
my_directory=r'd:\Users\jon98\Desktop\Nova_pasta\Nova_pasta'

# Take all txt filex except 'TODOS.txt'
files=glob.glob('*.txt')
files[:]=[os.path.join(my_directory,x) for x in files if 'todos.txt' not in x.lower()]
...
with open('TODOS.txt','r') as f:
    for i,line in enumerate(f):
        if i in lines_to_read:
            ...
            # already as float
            elastic_modulos[line.split('\t')[0].replace('_','-')]=float(line.split('\t')[1])
...
for i,(key,value) in enumerate(dfs.items()):
    ...
    if (value['Deformação']+i*0.5).max()>max_value_x:
        ...
        # last plot
        max_value_x = (value['Deformação']+i*0.5).max()
    ...
    plt.plot(value['Deformação']+i*0.5,value['Tensão'],linewidth=0.7,label=key)  
    
    # Plot modulus line. Also include offset. Same colour as the plot above
    plt.plot(value['Deformação']+i*0.5,y,linewidth=0.4,linestyle='dotted',color=plt.gca().lines[-1].get_color())
