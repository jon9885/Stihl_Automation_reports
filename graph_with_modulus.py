...
for i,(key,value) in enumerate(dfs.items()):
    ...
    plt.plot(value['Deformação']+i*0.2,value['Tensão'],linewidth=0.7,label=key)

    # Plot modulus line. Also include offset. Same colour as the plot above
    plt.plot(value['Deformação']+i*0.2,y,linewidth=0.4,linestyle='dotted',color=plt.gca().lines[-1].get_color())
