...
def delet_rows_after_break(data_frame):
    ...
    df2=data_frame['Deformação'].iloc[-100:].copy()
    ...
    index=df2[lambda x: x>=0.03].index[0]
...
mydirectory=r'd:\Users\jon98\Desktop\Nova_pasta'

# Identify all the .txt in the folder except todos.txt
files=glob.glob('*.txt')
files[:]=[os.path.join(mydirectory,x) for x in files if 'todos.txt' not in x.lower()]

# Name of the Excel table is gonna be the same as the folder
table_name=os.path.split(mydirectory)[-1]

# Initialize objects
writer=pd.ExcelWriter(os.path.join(mydirectory,table_name+'.xlsx'),engine='xlsxwriter')
...
for file in files:
    ...
    df.to_excel(writer,index=False,header=True,sheet_name=filename.split('.')[0],startrow=11)
    ...
    if file==files[0]: # Fisrt loop
        chart_sheet1=workbook.add_chart({'type':'scatter',
                                        'subtype':'straight'})
        chart_sheet1.set_x_axis({'name':'Deformação','min':0})
        ...
        chart_sheet2=workbook.add_chart({'type':'scatter',
                                        'subtype':'straight'})
        chart_sheet2.set_x_axis({'name':'Deformação','min':0})
...
worksheet.write(5,2,'=\''+os.path.split(files[0])[-1].split('.')[0]+'\'!B5',format_border2)
...
worksheet.write(7+len(files),2,'=AVERAGE(C8:C'+str(7+len(files))+')',format_border)
worksheet.write(8+len(files),2,'=STDEV(C8:C'+str(7+len(files))+')',format_border)

worksheet.write(7+len(files),3,'=AVERAGE(D8:D'+str(7+len(files))+')',format_border)
worksheet.write(8+len(files),3,'=STDEV(D8:D'+str(7+len(files))+')',format_border)

worksheet.write(7+len(files),4,'=AVERAGE(E8:E'+str(7+len(files))+')',format_border)
worksheet.write(8+len(files),4,'=STDEV(E8:E'+str(7+len(files))+')',format_border)

worksheet.write(7+len(files),5,'=AVERAGE(F8:F'+str(7+len(files))+')',format_border)
worksheet.write(8+len(files),5,'=STDEV(F8:F'+str(7+len(files))+')',format_border)

worksheet.write(7+len(files),6,'=AVERAGE(G8:G'+str(7+len(files))+')',format_border)
worksheet.write(8+len(files),6,'=STDEV(G8:G'+str(7+len(files))+')',format_border)
